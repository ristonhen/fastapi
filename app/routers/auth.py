...
def login(user_credentail: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(database.get_db)):
    ...
    user_data = {"id": user.id,
        ...
        "last_pwd_modified_date": user.last_pwd_modified_date,
        "description": user.description,
        "deviceid": user.deviceid,}
    access_token = oauth2.create_access_token(data={"user_id": user.id})
    response = schemas.Response(status=status.HTTP_200_OK, message= "tesing",data={"access_token": access_token,"user": user_data})
    return response
